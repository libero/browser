...
use Libero\JatsContentBundle\EventListener\CreateView\LinkListener;
...
final class LinkListenerTest extends TestCase
{
    ...
    public function it_sets_the_template_and_text_argument(string $xml, array $expectedText) : void
    {
        ...
        $context = ['qux' => 'quux'];
        $event = new CreateViewEvent($element, new View('@LiberoPatterns/link.html.twig', [], $context));
        $listener->onCreateView($event);
        $view = $event->getView();
...
XML
    ...
    public function it_does_nothing_if_it_is_something_else() : void
    {
        ...
        $event = new CreateViewEvent($element, new View('@LiberoPatterns/link.html.twig'));
        $listener->onCreateView($event);
        $view = $event->getView();
